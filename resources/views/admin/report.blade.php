    ...
    <div class="bg-white rounded-3xl shadow-2xl flex-1 flex flex-col p-8 w-full overflow-hidden border border-gray-100">
        ...
        <div class="space-y-4 overflow-y-auto pr-2 flex-1">
            @forelse($violations ?? [] as $violation)
                <div
                    class="grid grid-cols-3 items-center bg-gray-200/70 rounded-full py-4 px-8 shadow-sm border border-gray-300/50">
                    ...
                    <div class="flex items-center justify-end gap-3 font-extrabold text-sm tracking-wider">
                        ...
                        @if($status === 'resolved')
                            ...
                            <span
                                class="w-6 h-6 rounded-full bg-green-500 text-white flex items-center justify-center text-xs font-bold shadow">✓</span>
                        @else
                            <span class="text-gray-900 uppercase">{{ $status === 'dismissed' ? 'DISMISSED' : 'PENDING' }}</span>
                            <div
                                class="w-6 h-6 rounded-full bg-red-600 text-white flex items-center justify-center text-[10px] font-black shadow">
            ...
            @empty
                <div class="text-center py-16 text-gray-400 font-bold uppercase tracking-wider text-sm">
