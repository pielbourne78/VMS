...
<body class="bg-gray-100 antialiased flex flex-col h-screen overflow-hidden">
    ...
    <header class="header-gradient text-white px-6 py-4 shadow-md flex items-center justify-between z-20 flex-shrink-0">
        ...
        <nav class="flex items-center gap-6 text-sm font-bold">
            <a href="{{ route('admin.dashboard') }}" class="bg-white/20 px-5 py-2 rounded-full shadow-inner tracking-wide border border-white/30">DASHBOARD</a>
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <aside class="grc-red text-white w-72 flex flex-col py-6 shadow-2xl z-10 flex-shrink-0 relative">
            ...
            <nav class="flex flex-col space-y-1 font-bold text-sm">
                <a href="#" class="px-6 py-3.5 bg-red-900/50 border-l-4 border-white tracking-wide transition">Record Violation</a>
            ...
            <div class="absolute right-[-14px] top-64 bg-gray-200 text-gray-700 w-7 h-7 rounded-full flex items-center justify-center shadow-md cursor-pointer border border-gray-300 font-bold hover:bg-white transition">
                &lt;
            </div>
            ...
            <div class="mt-auto px-6">
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
                    <button type="submit" class="w-full bg-white text-grc-red font-black text-center py-2.5 rounded-full shadow-md hover:bg-gray-100 transition duration-150 text-sm tracking-widest">
                        LOG OUT
                    </button>
                </form>
            </div>
