...
<body class="antialiased min-h-screen overflow-x-hidden">
    ...
    <div class="dashboard-shell flex min-h-screen">
        <aside id="sidebar"
            ...
            <div class="px-6 flex flex-col items-center mb-8">
                <div class="relative mb-4">
                    <img src="{{ Auth::user()->profile_photo_url }}" alt="Student Avatar"
            ...
            <div class="px-6 space-y-3.5 text-sm font-semibold">
                <div
                    ...
                    <p class="truncate">{{ Auth::user()->course ?? 'Bachelor of Science in Information Technology' }}
                    </p>
                </div>
                <div
                    class="flex items-center gap-3 bg-red-800/40 px-4 py-2.5 rounded-xl border border-red-700/50 shadow-inner">
                ...
                <a href="{{ Route::has('code.of.discipline') ? route('code.of.discipline') : '#' }}"
                    ...
                    <span class="text-lg">📜</span>
                    <p class="truncate text-xs underline underline-offset-4 group-hover:opacity-90">ARTICLES VI CODE OF
                        DISCIPLINE</p>
                </a>
            </div>
        ...
        <div class="flex-1 flex flex-col bg-white">
            ...
            <header
                ...
                <nav class="flex items-center gap-6 text-base font-semibold tracking-tight">
                    ...
                    <a href="{{ route('dashboard') }}"
                        class="stat-btn-gradient text-white px-6 py-2.5 rounded-full shadow-inner tracking-wide">DASHBOARD</a>
                    <a href="{{ route('violation.monitoring') }}"
                        class="hover:text-red-200 transition tracking-wide">VIOLATION MONITORING</a>
                    <a href="#" class="hover:text-red-200 transition tracking-wide">REPORT</a>
                </nav>
                <div class="flex items-center gap-5">
                    <!-- Notification Bell -->
                    <div
                        class="bg-red-900/40 p-2.5 rounded-full cursor-pointer hover:bg-red-900/70 transition shadow-inner">
            ...
            <main class="flex-1 p-8 grid grid-cols-12 gap-8 overflow-y-auto bg-gray-50/50">
                ...
                @if(isset($violations) && count($violations) > 0)
                    ...
                    <section class="col-span-7 space-y-4">
                        @foreach($violations as $index => $violation)
                            <div
                                class="grid grid-cols-5 items-center bg-white border border-gray-200/80 rounded-xl p-3 text-center font-bold shadow-sm">
                                ...
                                <div class="flex justify-center">
                                    @if(($violation->severity ?? $violation['severity']) === 'major')
                                        <img src="https://raw.githubusercontent.com/carlvilla/resources/main/alert-red.png"
                                            alt="Major" class="h-8 w-8">
                                    @else
                                        <img src="https://raw.githubusercontent.com/carlvilla/resources/main/alert-yellow.png"
                                            alt="Minor" class="h-8 w-8">
                                    @endif
                                </div>
                                <div class="flex flex-col items-center gap-1">
                                    ...
                                    @if(($violation->status ?? $violation['status']) === 'pending')
                                        <button
                                            class="bg-grc-red text-white text-xs px-4 py-1 rounded-full font-semibold shadow hover:bg-red-700">PENDING</button>
                                    @else
                                        <button
                                            class="penalty-btn-gradient text-white text-xs px-4 py-1 rounded-full font-semibold shadow">PENALTY
                                            QUIZ</button>
                                    @endif
                                </div>
                            </div>
                    ...
                    <section class="col-span-5 space-y-6">
                        <div class="flex items-center gap-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-200/80">
                            <img src="https://raw.githubusercontent.com/carlvilla/resources/main/alert-yellow.png"
                                alt="Total" class="w-16 h-16">
                            <div>
                                <h3 class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">TOTAL VIOLATIONS
                                </h3>
                        ...
                        <div class="flex gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-200/80">
                            <img src="https://raw.githubusercontent.com/carlvilla/resources/main/alert-yellow.png"
                                alt="Minor" class="w-12 h-12 flex-shrink-0 mt-1">
                            <div>
                                ...
                                <p class="text-gray-600 text-xs leading-relaxed font-medium">
                                    Small-scale infractions that disrupt order but do not cause significant harm or safety
                                    risks. Often treated as correctable behaviors.
                                </p>
                            </div>
                        </div>

                        <div class="flex gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-200/80">
                            <img src="https://raw.githubusercontent.com/carlvilla/resources/main/alert-red.png" alt="Major"
                                class="w-12 h-12 flex-shrink-0 mt-1">
                            <div>
                                <h3 class="text-grc-red text-xl font-extrabold tracking-tight mb-2">MAJOR OFFENSE</h3>
                                <p class="text-gray-600 text-xs leading-relaxed font-medium">
                                    Serious violations threatening the integrity, safety, or legal standing of the
                                    institution involving breaches of trust, ethics, or law.
                                </p>
                            </div>
                        </div>
                    </section>
                @else
                    ...
                    <section class="col-span-7 flex flex-col">
                        <div
                            class="bg-white rounded-2xl shadow-sm border border-gray-200/80 p-8 flex-1 flex flex-col items-center justify-center text-center">
                            <div
                                class="w-20 h-20 bg-green-50 text-green-500 rounded-full flex items-center justify-center text-4xl mb-4 shadow-inner">
                                🎉
                            </div>
                            ...
                            <p class="text-gray-500 text-sm max-w-md leading-relaxed">
                                Fantastic job! Your record is currently clean with zero reported violations. Keep up the
                                great conduct and discipline inside the campus.
                            </p>
                        </div>
                    </section>
                    ...
                    <section class="col-span-5 space-y-6">
                        <div
                            class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200/80 flex items-center justify-between">
                            <div>
                                <h3 class="text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">TOTAL VIOLATIONS
                                </h3>
                            ...
                            <div
                                class="w-14 h-14 bg-red-50 rounded-2xl flex items-center justify-center text-grc-red shadow-inner font-bold text-xl">
                                🛡️
                            </div>
                        </div>
                        ...
                        <div class="bg-gradient-to-br from-red-50 to-white p-6 rounded-2xl shadow-sm border border-red-100">
                            ...
                            <p class="text-gray-600 text-sm leading-relaxed font-medium">
                                Global Reciprocal Colleges values a safe, orderly, and respectful learning environment.
                                Always wear your complete school uniform, maintain proper grooming, and respect fellow
